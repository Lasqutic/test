const validate = ({ data, name, instance }) => {
    ...
    if (typeof payload.name !== 'string') {
        throw new Error(`${name}: payload.name should be a string`);
    }
    ...
    if (typeof payload.email !== 'string') {
        throw new Error(`${name}: payload.email should be a string`);
    }
    ...
    if (typeof payload.password !== 'string') {
        throw new Error(`${name}: payload.password should be a string`);
    }
};
